...
#is the primary key autogenerated? (e.g. mysql AUTO_INCREMENT, postgres SERIAL)
#if so, we don't insert a null value, this will throw errors with postgres
sub PRIMARY_KEY_AUTOGENERATED {0}
...
$SQL = {
  ...
  INSERT => sub {
    ...
    return $SQL_CACHE->{$class}{INSERT} ||= do {
      ...
      $pk = $class->dbh->quote_identifier($pk) if $class->QUOTE_IDENTIFIERS;
      @columns = grep { $_ ne $pk } @columns
        if $class->PRIMARY_KEY_AUTOGENERATED;
...
sub insert {
  ...
  my @columns = @{$class->_UNQUOTED->{COLUMNS}};
  @columns = grep { $_ ne $pk } @columns
    if $class->PRIMARY_KEY_AUTOGENERATED;
  ...
  return $self->{data}{$pk}
    ||= $self->dbh->last_insert_id(undef, undef, $self->_UNQUOTED->{TABLE}, $pk);
