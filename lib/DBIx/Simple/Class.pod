...
=head2 PRIMARY_KEY_AUTOGENERATED

A true/false value.  If set to true, the PRIMARY_KEY is assumed to be autogenerated
by a sequence (e.g. MySQL AUTO_INCREMENT or PostgreSQL SERIAL), and a NULL will not be
written to that column on an insert.  Necessary as PostgreSQL errors on NULL inputs to
a NOT NULL column with a DEFAULT.
Default is false.

    use constant PRIMARY_KEY_AUTOGENERATED => 1;
